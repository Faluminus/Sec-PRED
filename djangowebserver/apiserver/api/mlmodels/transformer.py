...
indices_to_keys = {v: k for k, v in dssp8_structures.items()}
...
class Transformer(nn.Module):
    ...
    def generate_mask(self, src, tgt):
        ...
        nopeak_mask = (1 - torch.triu(torch.ones(1, seq_len, seq_len), diagonal=1)).bool().to(src.device)
...
class TransformerPredict(nn.Module):
    r"""
    This module is used for predicting protein secondary structure in secpred api
    input_ : expects amino acid sequence
    start(): runs prediction and returns indices
    ...
    def start(self):
        ...
        with torch.no_grad():
            output = self.transformer(self.input)
            self.indices = torch.argmax(output, dim=2).squeeze(0)

    def translate_to_secondary_structure(self):
        seqPredicted = ''
        for indice in self.indices:
            seqPredicted += indices_to_keys[indice]
