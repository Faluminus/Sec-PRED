import { error } from '@sveltejs/kit';


async function fetchPDB(input_search) {
    const FASTA = await fetch('https://www.rcsb.org/fasta/entry/'+input_search+'/download',{
        method:"GET",
        mode:"cors",
        headers: {
            'Access-Control-Allow-Origin':'*'
        }
    })
    .then(async (response)=>{
        const data_splited = (await response.text()).split('\n');
        for(let row of data_splited){
            if(row.split('')[0] != '>'){
            return row;
            }
        }
        console.log(data_splited)
    })
    .catch((exception)=>{
        console.log(exception)
    })
    return { FASTA }
}

export async function POST({ request }){
    const { input_search } = await request.json();
    console.log(input_search)
    const FASTA = await fetchPDB(input_search);
    return new Response(JSON.stringify({ FASTA }), {
      headers: { 'Content-Type': 'application/json' }
    });
  }