...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.LSTM(128, return_sequences=True, activation='tanh', recurrent_activation='relu',unroll=False, recurrent_dropout=0, use_bias=True, dropout=0.2, kernel_regularizer=L2(0.001)),
    tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(max_features, activation='sigmoid'))
])

optimizer = tf.keras.optimizers.legacy.Adam(learning_rate=0.0001 ,clipnorm=0.5)
...
model.evaluate(src_input_ids_np_test, tgt_input_ids_np_test)
