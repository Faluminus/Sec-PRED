...
class Controller:
    ...
    def fetchPDB(self, id):
        if id == None:
             return make_response(jsonify({"output": "No pdb id provided"}), 422)
        amino_acid_seq = requests.get('https://www.rcsb.org/fasta/entry/'+id+'/download')
        data_splited = str(amino_acid_seq.text).splitlines()
        for row in data_splited:
            if row[0] != '>':
                return make_response(jsonify({"output": row}), 200)
        return make_response(jsonify({"output": "Uknown error"}))
